import ASC.FiniteCoxeterComplex  
...
(dual r).toPartialOrder.toPreorder ∈ AFLOPartitions_positive μ ↔ (∀ (a : α), μ a ≥ 0):= by
  ...
  . intro hr a 
    ...
    set b := WellFounded.min (@Finite.to_wellFoundedGT α _ r.toPartialOrder.toPreorder).wf Set.univ hne  
    ...
    | ⟨c, hc⟩ => have hX : X ∈ preorderToPowersetFinset ⟨(dual r).toPartialOrder.toPreorder, (AFLOPartitions_is_everything 
                   ...
                   . intro d e hde 
                     ...
                     intro hdb 
                     rw [hdb] at hde 
                     have h := WellFounded.not_lt_min (@Finite.to_wellFoundedGT α _ r.toPartialOrder.toPreorder).wf Set.univ hne 
                       (Set.mem_univ e)
...
(WeakBruhatOrder_facets_WeightedComplex μ hsum r).le s t  → so.le s t) : IsShellingOrder so :=  
  ...
  μ hsum hmon s t hst)) (@Finite.to_wellFoundedLT _ (Finite.of_injective (fun s => s.1) 
  (fun s t hst => by rw [SetCoe.ext_iff] at hst; exact hst)) so.toPartialOrder.toPreorder).wf
