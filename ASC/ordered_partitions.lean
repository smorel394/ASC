import ASC.preorder_to_powerset  
...
r.le a b → r.le a c → s.le b c := by
  set a:= (WellFounded.min (@Finite.to_wellFoundedGT α (Finite.of_fintype α) (PreorderofLinearOrder r)).wf Set.univ Set.univ_nonempty)
  exists a
  intro b c hab hac 
  have habeq : a = b := by
    apply r.le_antisymm a b hab
    exact @WellFounded.min_le α (dual r) (@Finite.to_wellFoundedGT α (Finite.of_fintype α) (PreorderofLinearOrder r)).wf b Set.univ 
      (Set.mem_univ b) Set.univ_nonempty 
  have haceq : a = c := by 
    apply r.le_antisymm a c hac
    exact @WellFounded.min_le α (dual r) (@Finite.to_wellFoundedGT α (Finite.of_fintype α) (PreorderofLinearOrder r)).wf c Set.univ 
      (Set.mem_univ b) Set.univ_nonempty 
  rw [←habeq, ←haceq]
...
| ⟨a, b, habinv⟩ => 
    ...
    have hwf := (@Finite.to_wellFoundedLT (A × A) _ _).wf  
