import ASC.FiniteOrderedPartitions 
import ASC.Shellability
import ASC.WeakBruhatOrder 
import ASC.EulerPoincareCharacteristic 
...
(WeakBruhatOrder_facets r).le s t  → so.le s t) : IsShellingOrder so :=  
  ShellableofDecomposable (CoxeterComplex_is_decomposable r) (fun s t hst => hcomp (WeakBruhat_compatible_with_DF r s t hst))
   (@Finite.to_wellFoundedLT _ (Finite.of_injective (fun s => s.1) (fun s t hst => by rw [SetCoe.ext_iff] at hst; exact hst)) 
   so.toPartialOrder.toPreorder).wf 
