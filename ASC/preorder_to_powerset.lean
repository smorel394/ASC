...
∃ (a : α), ∀ (b : α), ¬(s.lt a b) := by 
  ...
  exists ofAntisymmetrization s.le (WellFounded.min (@Finite.to_wellFoundedGT _ hfin _).wf _ hane)
  intro b 
  rw [←toAntisymmetrization_lt_toAntisymmetrization_iff, toAntisymmetrization_ofAntisymmetrization]  
  exact WellFounded.not_lt_min (@Finite.to_wellFoundedGT _ hfin _).wf _ hane (Set.mem_univ (toAntisymmetrization s.le b)) 
